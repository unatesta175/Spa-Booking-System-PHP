...
$start = "10:00";
$end = "19:00";
// Retrieve duration data from the URL and store it in the $duration_sv variable
$duration_sv = isset($_GET['duration']) ? $_GET['duration'] : '';

// Filter timeslots based on the specified conditions
function timeslotsWithDuration($duration_sv, $cleanup, $start, $end, $existingBookings)
{
    ...
    for ($currentTime = $startTime; $currentTime < $endTime; $currentTime += ($duration_sv + $cleanup) * 60) {
        // Calculate end time for the current timeslot
        $endTimeSlot = $currentTime + $duration_sv * 60;
...
<body>
   ...
   <div class="section">
    ...
    <?php
 <?php
 // Filter timeslots based on the duration_sv variable
 $timeslots = timeslotsWithDuration($duration_sv, $cleanup, $start, $end, $existingBookings);
 foreach ($timeslots as $ts) {
     ...
     <div class="col-md-2">
         <div class="form-group m-3">
             ...
             if ($ts['overlap']==false && strtotime($ts['timeslot']) + $duration_sv * 60 <= strtotime("14:00") && strtotime(explode(' - ', $ts['timeslot'])[1]) > strtotime("19:00")) { ?>
