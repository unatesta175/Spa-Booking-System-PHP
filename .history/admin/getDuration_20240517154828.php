...
$select_accounts = $conn->prepare("SELECT * FROM `services` WHERE id = ? ");
	...
	if ($select_accounts->rowCount() > 0) {
		while ($row = $select_accounts->fetch(PDO::FETCH_ASSOC)) {
			...
			echo "<input   hidden id='servicename' readonly type='text' value=" . $row['name']+30 ." >";
	...
	if ($select_accounts1->rowCount() > 0) {
		while ($row1 = $select_accounts1->fetch(PDO::FETCH_ASSOC)) {
			...
			echo "<input   hidden id='clientname' readonly type='text' value=" . $row1['duration']+30 ." >";

		}
	}

	$select_accounts2 = $conn->prepare("SELECT * FROM `services` WHERE id = ? ");
	$select_accounts2->execute([$_POST['servicev1']]);
	if ($select_accounts2->rowCount() > 0) {
		while ($row2 = $select_accounts2->fetch(PDO::FETCH_ASSOC)) {

			echo "<input   hidden id='clientname' readonly type='text' value=" . $row2['duration']+30 ." >";
