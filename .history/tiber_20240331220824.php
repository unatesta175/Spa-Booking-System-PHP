...
function build_calendar($month, $year)
{
    ...
    $daysOfWeek = array('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'); // Adjusted starting from Monday
    $firstDayOfMonth = mktime(0, 0, 0, $month, 1, $year);
    $numberDays = date('t', $firstDayOfMonth);
    $dateComponents = getdate($firstDayOfMonth);
    $monthName = $dateComponents['month'];
    $dayOfWeek = $dateComponents['wday']; // Get the day of the week for the 1st of the month
    $dayOfWeek = ($dayOfWeek == 0) ? 6 : $dayOfWeek - 1; // Adjust the day of the week to start from Monday (0-indexed)
    ...
    while ($currentDay <= $numberDays) {
        ...
        if (in_array($date, $bookings)) {
            $calendar .= "<td class='$today'><h4>$currentDayRel</h4><a class='btns btn-danger btn-xs'>Booked</a></td>";
        } else {
            $calendar .= "<td class='$today'><h4>$currentDayRel</h4><a class='btns btn-success btn-xs'>Book</a></td>";
        }
...
th, td {
  width: 14.28%; /* Each column takes 14.28% of the table width (100% / 7) */
  padding: 10px;
  text-align: center;
  font-size: 1.3rem;
}
...
.today {
  background-color: #ff0a85 !important;
}
  </style>
</head>
