...
function build_calendar($month, $year)
{
  ...
  // HTML table for displaying the calendar
  ...
  while ($currentDay <= $numberDays) {
    ...
    // Construct the DateTime object for the current date
    ...
    $today = ($currentDate->format('Y-m-d') === $dateToday) ? 'today' : '';

    // Check if the current date is booked or if it's a specific day
if ($dayName === 'tuesday' && $currentDate < new DateTime()) {
      $calendar .= "<td><h4>$currentDay</h4><button class='btns btn-secondary btn-xs'>Spa Off Day</button></td>";
    } elseif ($currentDate < new DateTime()) {
      $calendar .= "<td><h4>$currentDay</h4><button class='btns btn-danger btn-xs'>N/A</button></td>";
    } else {
      // Display an available date
      $calendar .= "<td class='$today'><h4>$currentDay</h4><a href='timeslot.php?date=" . $currentDate->format('Y-m-d') . "'class='btns btn-success btn-xs'>Book</a></td>";
    }
