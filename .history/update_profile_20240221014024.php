...
if (isset($_POST['submit'])) {
   ...
   $update = $conn->prepare("UPDATE `clients` SET name = ?, ic = ?, email = ?, password = ?, phoneno = ? , datebirth= ? , address= ? , maritalstat= ?, religion= ?, occupation= ?, company= ?, fb= ?, ig= ?, allergy= ? WHERE id = ?");
   $update->execute([$name, $ic,$email, $password, $phoneno, $datebirth, $address, $maritalstat,$religion,$occupation,$company,$fb,$ig, $allergy, $user_id]);
