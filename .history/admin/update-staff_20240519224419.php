...
if (isset($_POST['update'])) {
   ...
   $update = $conn->prepare("UPDATE `staffs` SET name = ?, ic = ?, phoneno = ? , datebirth= ? , address= ? , updated_at = ? WHERE id = ?");
   $update->execute([$name, $ic, $phoneno, $datebirth, $address,$defaultDateTime ,$pid]);
      ...
      ?>
         ...
         <input value="<?php echo $address; ?>" type="text" id="address" name="address" required placeholder="Masukkan alamat kediaman anda" maxlength="70" class="box">
               ...
               <div class="flex-btn">
                  ...
                  <a href="staff-accounts.php" class="option-btn">Kembali</a>
               </div>
            </form>
