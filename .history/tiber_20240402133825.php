...
function build_calendar($month, $year)
{
  ...
  // Prepare SQL statement to fetch bookings for the given month and year
     ...
     $stmtStaff = $conn->prepare("SELECT * FROM staffs");
     $stmtStaff->execute();
 
     // Initialize variables
     $staffOptions = '';
     $firstRoom = 0; // Initialize the first staff ID
 
     // Generate options for staff select input
     while ($rowStaff = $stmtStaff->fetch(PDO::FETCH_ASSOC)) {
         if (empty($firstRoom)) {
             $firstRoom = $rowStaff['id']; // Set the first staff ID
         }
         $staffOptions .= "<option value='" . $rowStaff['id'] . "'>" . $rowStaff['name'] . "</option>";
     }
 
     // Prepare SQL statement to fetch bookings for the given month, year, and staff ID
     $stmt = $conn->prepare("SELECT * FROM bookings WHERE MONTH(date) = ? AND YEAR(date) = ? AND staff_id = ?");
     $stmt->execute([$month, $year, $firstRoom]); // Use the first staff ID by default
