      ...
      <div class="container">
         ...
         <div class="row"
            ...
            <?php else: ?>
               <div class="col-md-12">
                  <br>
                  <div class='alert alert-secondary'>Sila pilih tarikh tempahan dan juga Pakar Terapi untuk melihat slot
                     tempahan masa yang terbuka!</div>
               </div>
            <?php endif; ?>
   ...
   <div id="myModal" class="modal fade" role="dialog">
      <div class="modal-dialog">
         ...
         <div style="border-radius:25px;" class="modal-content">
           ...
           $result = $conn->prepare("SELECT * FROM `staffs` where =?");
            $result->execute([$_GET['staff_id']]);
                           if ($result->rowCount() > 0) {
                              while ($row = $result->fetch(PDO::FETCH_ASSOC)) { ?>
                                 ?>
                                 <?php echo $row['name'];
                           ...
                           <div class="inputBox ">
                              <label>Pakar Terapi :<span style="color: red;"></span></label>
                              <select class="box" required placeholder="" name="staff_id_modal">
                                 <option value="" selected hidden>Sila Pilih Pakar Terapi Anda</option>
      ...
      $(document).ready(function () {
         $(".book").click(function (event) {
            ...
            if (overlap) {
               ...
               $("#slot").html(timeslot);
               $("#timeslot").val(timeslot); // Set the timeslot value in a hidden input field for form submission

               // Extract the start time from the timeslot
               var startTime = timeslot.split(' - ')[0];

               // Set the extracted start time as the value of the time input field in the modal
               $("#time").val(startTime);
