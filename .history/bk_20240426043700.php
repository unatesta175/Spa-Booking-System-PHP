...
.btns.selected {
    background-color: #4CAF50; /* Darker green for selected timeslot */
    color: white;
    border: 3px solid #4CAF50; /* Matching border for clarity */
}
      .baby {
...
<body>
            ...
            <div class="col-md-4 col-sm-6">
               ...
               <form style="box-shadow: none;" action="" method="post">
                     ...
                     <div class="inputBox">
                        ...
                        <input aria-readonly style="font-family: 'Gilroymedium', sans-serif;" readonly
                           style="font-size:1.3rem;" type="text" value=""
                           name="timeslotz" id="timeslotz" class="box" placeholder="Slot Masa">
                     </div>
   ...
   <script>
document.addEventListener('DOMContentLoaded', function () {
    const timeslotButtons = document.querySelectorAll('.book.btn-success'); // Select only unbooked timeslot buttons

    timeslotButtons.forEach(button => {
        button.addEventListener('click', function () {
            // Remove the selected class from all buttons
            document.querySelectorAll('.book').forEach(btn => btn.classList.remove('selected'));

            // Add the selected class to the clicked (and available) button
            this.classList.add('selected');

            // Extract the start time from the button's data attribute
            const startTime = this.dataset.timeslot.split(' - ')[0];

            // Get the duration from the duration input and parse it
            const durationText = document.getElementById('duration').value;
            const hours = parseInt(durationText.match(/(\d+)\s*Jam/)?.[1]) || 0;
            const minutes = parseInt(durationText.match(/(\d+)\s*Minit/)?.[1]) || 0;

            // Calculate the end time by adding the duration to the start time
            const endTime = addTime(startTime, hours, minutes);

            // Update the timeslotz input with the formatted timeslot range
            document.getElementById('timeslotz').value = `${startTime} - ${endTime}`;
        });
    });
});

// Helper function to add time and format the result
function addTime(startTime, hoursToAdd, minutesToAdd) {
    const timeParts = startTime.match(/(\d+):(\d+)\s*(AM|PM)/);
    const date = new Date();
    date.setHours(parseInt(timeParts[1]) % 12 + (timeParts[3] === 'PM' ? 12 : 0));
    date.setMinutes(parseInt(timeParts[2]));
    date.setHours(date.getHours() + hoursToAdd);
    date.setMinutes(date.getMinutes() + minutesToAdd);

    // Format the new time in AM/PM format
    const hours = date.getHours() % 12 || 12; // Adjust 0 hour to 12 for 12-hour clock
