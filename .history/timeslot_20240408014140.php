...
function timeslots($duration, $cleanup, $start, $end, $existingBookings)
{
   ...
   for ($currentTime = $startTime; $currentTime < $endTime; $currentTime += ($duration + $cleanup) * 60) {
      ...
      foreach ($existingBookings as $booking) {
         ...
         $bookingEndTime = $bookingStartTime + $booking['duration'] * 60;
      ...
      // If the timeslot does not overlap with any existing booking, add it to the list of available timeslots
      if (!$overlap) {
         // Format timeslot start and end times in 24-hour format
         $slotStartTime = date('H:i', $currentTime);
         $slotEndTime = date('H:i', $endTimeSlot);
         // Add timeslot to the array and capitalize AM or PM
         $slots[] = $slotStartTime . ' - ' . $slotEndTime . ' ' . date('A', $currentTime);
      }
   }
   ...
   <div class="section">
      ...
      <div class="container">
         ...
         <div class="row"
            ...
            $bookedSlots = array_fill_keys(array_column($existingBookings, 'timeslot'), true);

            foreach ($timeslots as $ts) {
               $isBooked = isset($bookedSlots[$ts]);
               ?>
               <div class="col-md-2">
                  <div class="form-group m-3">
                     <button class="btns <?php echo $isBooked ? 'btn-danger' : 'btn-success'; ?> book"
                        data-timeslot="<?php echo $ts; ?>" <?php echo $isBooked ? 'disabled' : ''; ?>>
                        <?php echo $ts; ?>
                     </button>
                  </div>
               </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Get all elements with the class 'timeslot-btn'
        var timeslotButtons = document.querySelectorAll('.timeslot-btn');

        // Loop through each timeslot button
        timeslotButtons.forEach(function (button) {
            // Get the timeslot value from the 'data-timeslot' attribute
            var timeslot = button.getAttribute('data-timeslot');

            // Check if the timeslot is booked
            var isBooked = <?php echo json_encode(in_array($ts, array_column($existingBookings, 'timeslot'))); ?>;

            // Update the button color based on booking status
            button.classList.toggle('btn-danger', isBooked);
            button.classList.toggle('btn-success', !isBooked);

            // Disable the button if it's booked
            if (isBooked) {
            ...
            button.addEventListener('click', function () {
                if (!isBooked) {
                    ...
                    console.log('Booking timeslot: ' + timeslot);
                    // Optionally, you can open a modal or perform any other action
                }
            });
        });
    });
