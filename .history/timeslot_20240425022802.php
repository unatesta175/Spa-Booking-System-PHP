...
if (isset($_POST['submit'])) {
   ...
   foreach ($existing_bookings as $booking) {
      if ($booking['staff_id'] == $staff_id) {
         ...
         // User booked the same timeslot with a different staff
         // Check if there is any overlapping booking for the selected staff
         $stmt = $conn->prepare("SELECT * FROM bookings WHERE staff_id = ? AND date = ? AND ((timeslot <= ? AND timeslot_end >= ?) OR (timeslot <= ? AND timeslot_end >= ?) OR (timeslot >= ? AND timeslot_end <= ?))");
         $stmt->execute([$staff_id, $date, $timeslot, $timeslot, $timeslot, $timeslot, $timeslot, $timeslot]);
         if ($stmt->rowCount() > 0) {
...
<body>
 ...
 <div class="section">
   ...
   <div class="container">
      <br>
      <h1 class="text-center">Tempah untuk tarikh : <?php echo date('m/d/Y', strtotime($date)); ?></h1>
      <div class="row">
         <div class="col-md-12">
            ...
            <div class="col-md-2">
               <div class="form-group m-3">
                  <?php if (in_array($ts, $bookings)) { ?>
                     <button class="btns btn-danger book" data-timeslot="<?php echo $ts; ?>"><?php echo $ts; ?></button>
                  <?php } else { ?>
                     <button class="btns btn-success book" data-timeslot="<?php echo $ts; ?>"><?php echo $ts; ?></button>
                  <?php } ?>
               </div>
            </div>
         <?php } ?>
