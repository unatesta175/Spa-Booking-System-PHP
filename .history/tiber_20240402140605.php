...
function build_calendar($month, $year, $selected_room)
{
  ...
  // Prepare SQL statement to fetch rooms
  $stmt = $conn->prepare("SELECT * FROM staffs");
  $rooms_options = "";
  $stmt->execute();

  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $rooms_options .= "<option value='".$row['id']."'>".$row['name']."</option>";
  }

  // Prepare SQL statement to fetch bookings for the selected month, year, and room
  $stmt = $conn->prepare("SELECT * FROM bookings WHERE MONTH(date) = ? AND YEAR(date) = ? AND staff_id = ?");
  $stmt->execute([$month, $year, $selected_room]);
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-12 bg-light">
          ...
          $selected_room = isset($_GET['room']) ? $_GET['room'] : 0;

          echo build_calendar($month, $year, $selected_room);
