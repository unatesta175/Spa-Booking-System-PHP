...
$type = ''; // To determine the type of alert
...
$insert_user = $conn->prepare("INSERT INTO `clients`(name,ic, email, password,phoneno,datebirth,address,maritalstat,religion,occupation,company,fb,ig,allergy,datetimeregistered,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
         $insert_user->execute([$name,$ic,$email,$cpass,$phoneno,$datebirth,$address,$maritalstat,$religion,$occupation,$company,$fb,$ig, $allergy, $currentDateTime,$updated_at]);
         $message = 'Anda sudah berjaya daftar!';
         $messageTitle = 'Berjaya!';
         $type = 'success';
      }
   }
