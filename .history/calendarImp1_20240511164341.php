   ...
   <section>
      <div class="container" style="background-color:white; border-radius: 25px; padding:15px; box-shadow: var(--box-shadow);">
         ...
         <div class="container" style="margin-top:1rem;margin-bottom:1rem;">
               <a href="record.php" class="btns btn-secondary"><i class="fas fa-arrow-left"></i>Lihat senarai tempahan anda</a>
            </div>
         <div id='calendar'>
         </div>
      </div>
   ...
   <script>
      document.addEventListener('DOMContentLoaded', function() {
         ...
         var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            // Check if there are any events in this timeslot
            var eventsInSlot = calendar.getEvents().filter(function(event) {
                return event.start <= info.date && event.end >= info.date;
            });

            // If no events found, display a tooltip
            if (eventsInSlot.length === 0) {
                var eventStartTime = info.date;
                var startTimeFormatted = new Date(eventStartTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
                var tooltip = document.createElement('div');
                tooltip.textContent = 'Start Time: ' + startTimeFormatted;
        ...
        eventMouseLeave: function(info) {
            if (info.el.tooltip) {
                info.el.tooltip.parentNode.removeChild(info.el.tooltip);
