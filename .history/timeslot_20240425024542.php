...
if (isset($_GET['date'])) {
   ...
   // Prepare SQL statement to featch bookings for the given month and year
...
<script>
   $(document).ready(function () {
      // Function to handle onchange event for staff selection
      $("#staff_id").change(function () {
         var staffId = $(this).val();
         fetchTimeslots(staffId);
      });

      // Function to handle onchange event for service selection
      $("#service_id").change(function () {
         var serviceId = $(this).val();
         fetchServiceDuration(serviceId);
      });

      // Function to fetch timeslots based on selected staff ID
      function fetchTimeslots(staffId) {
         $.ajax({
            url: 'fetch_timeslots.php',
            type: 'GET',
            data: { staff: staffId },
            success: function (response) {
               // Update the timeslots based on the response
               // You can update the HTML to display the fetched timeslots
            },
      ...
      function fetchServiceDuration(serviceId) {
         $.ajax({
            url: 'fetch_service_duration.php',
            type: 'GET',
            data: { service: serviceId },
            success: function (response) {
               // Update the service duration based on the response
               // You can update the HTML to display the fetched duration
            },
            error: function (xhr, status, error) {
               console.error(xhr.responseText);
            }
