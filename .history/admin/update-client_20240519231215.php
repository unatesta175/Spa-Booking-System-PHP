...
update = $conn->prepare("UPDATE `clients` SET name = ?, ic = ?, phoneno = ? , datebirth= ? , address= ? , maritalstat= ?, religion= ?, occupation= ?, company= ?, fb= ?, ig= ?, allergy= ? updated_at = ? WHERE id = ?");
   $update->execute([$name, $ic, $phoneno, $datebirth, $address, $maritalstat,$religion,$occupation,$company,$fb,$ig, $allergy,$currentDateTime ,$pid]);
...
<!-- 
    ...
    <span for="maritalstat">Status Perkahwinan<span style="color: red;"></span></span>
    ...
    <span for="religion">Agama<span style="color: red;"></span></span>
<select id="religion" name="religion" required class="box" onchange="showOtherField()">
<option value="<?php echo $fetch_products['religion']; ?>" selected hidden><?php echo $fetch_products['religion']; ?></option>
...
<span for="occupation">Pekerjaan<span style="color: red;"></span></span>
<select id="occupation" name="occupation" required class="box" onchange="toggleOtherOccupationField()">
<option value="<?php echo $fetch_products['occupation']; ?>" selected hidden><?php echo $fetch_products['occupation']; ?></option>
...
<input type="text" id="otherOccupation" name="otherOccupation" style="display:none;" placeholder="Masukkan pekerjaan anda" class="box">
    ...
    <span for="company">Nama Syarikat</span>
    <input value="<?php echo $fetch_products['company']; ?>" type="text" id="company" name="company" placeholder="Masukkan nama syarikat anda jika ada" maxlength="70" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
    
    <span for="fb">Link Facebook : <span style="color: gray;">Jika ada*</span></span>
    <input value="<?php echo $fetch_products['fb']; ?>" type="text" id="fb" name="fb" placeholder="Masukkan username facebook anda jika ada" maxlength="70" class="box" oninput="this.value = this.value.replace(/\s/g, '')">

    <span for="ig">Link Instagram  : <span style="color: gray;">Jika ada*</span></span>
    <input value="<?php echo $fetch_products['ig']; ?>" type="text" id="ig" name="ig" placeholder="Masukkan username instagram anda jika ada" maxlength="70" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
   
    <span for="allergy">Alahan : <span style="color: gray;">Jika ada*</span></span>
    <input value="<?php echo $fetch_products['allergy']; ?>" type="text" id="allergy" name="allergy"  required placeholder="Nyatakan Alahan anda jika ada" maxlength="70" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
