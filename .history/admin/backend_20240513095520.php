...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($select_user->rowCount() > 0) {
        ...
        $select_accounts = $conn->prepare("INSERT INTO `clients`(name, email, password,phoneno,datebirth,address,maritalstat,religion,occupation,company,fb,ig,allergy,datetimeregistered) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $select_accounts->execute([$email]);
            $row = $select_user->fetch(PDO::FETCH_ASSOC);
