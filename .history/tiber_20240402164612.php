...
<body>
  ...
  <script>
    ...
    $(document).on('change', '#staff_select', function () {
      ...
      var month = '<?php echo $month; ?>'; // Get the current month
      var year = '<?php echo $year; ?>'; // Get the current year
      // Send AJAX request to calendar.php with selected staff ID, month, and year
      $.ajax({
        url: "calendar.php",
        type: "POST",
        data: { 'month': month, 'year': year, 'staff': staffId },
        success: function (data) {
          // Replace the content of the calendar div with the updated calendar data
          $("#calendar").html(data);
        }
      });
    });
