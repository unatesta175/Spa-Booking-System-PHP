...
// Modify the timeslots function to accept duration_sv as a parameter
function timeslots($duration, $cleanup, $start, $end, $existingBookings, $duration_sv)
{
    ...
    for ($currentTime = $startTime; $currentTime < $endTime; $currentTime += ($duration + $cleanup) * 60) {
        ...
        // Check if the current timeslot overlaps with any existing booking or specific times
        ...
        // Check if the current timeslot overlaps with specific times (staff resting time, etc.)
        // Adjust the specific times as needed
        $currentHour = date('H', $currentTime);
        if (($currentHour >= 13 && $currentHour < 14) || $currentHour >= 19) {
            $overlap = true;
        }
        ...
        $slots[] = array(
            ...
            'overlap' => $overlap // Store whether the timeslot overlaps with an existing booking or specific times
        );
    }
...
<body>
   ...
   <div class="section">
    ...
    <?php
$timeslots = timeslots($duration, $cleanup, $start, $end, $existingBookings,$duration_sv);
foreach ($timeslots as $ts) {
    ...
    <!-- <div class="col-md-2">
        <div class="form-group m-3">
            ...
            if ($ts['overlap']) { ?>
                <button disabled class="btns baby btn-danger book" data-timeslot="<?php echo $ts['timeslot']; ?>">
                    <?php echo $start_time; ?> 
                </button>
            <?php } else { ?>
...
document.addEventListener('DOMContentLoaded', function () {
    // Check for service ID query parameter in the URL
    var urlParams = new URLSearchParams(window.location.search);
    var selectedServiceId = urlParams.get('service');
    if (selectedServiceId) {
        // If service ID query parameter is present, set the selected option
        document.getElementById('service_id').value = selectedServiceId;
    }
});

document.addEventListener('DOMContentLoaded', function () {
    // Check for duration query parameter in the URL
    var urlParams = new URLSearchParams(window.location.search);
    var selectedDuration = urlParams.get('duration');
    if (selectedDuration) {
        // If duration query parameter is present, format the duration value
        var formattedDuration = formatDurations(selectedDuration);
        // Set the formatted duration as the value of the duration input
        document.getElementById('duration').value = formattedDuration;
    }
});

function formatDurations(duration) {
    // Convert the duration to an integer
    duration = parseInt(duration);

    // If the duration is exactly divisible by 60, it's in hours
    if (duration % 60 === 0) {
...
document.getElementById('duration').addEventListener('input', function() {
    // Get the value of the duration input
    ...
    if (currentUrl.indexOf('?') !== -1) {
        // URL already contains a query string, so append the duration parameter
        window.history.replaceState(null, null, currentUrl + '&duration=' + durationValue);
    } else {
        // URL does not contain a query string, so add the query string with the duration parameter
        window.history.replaceState(null, null, currentUrl + '?duration=' + durationValue);
    }
});

// Store the selected service ID and duration value in localStorage when they are changed
...
document.getElementById('service_id').addEventListener('change', function () {
    ...
    // Get the duration from the selected option's data-duration attribute
    var duration = parseInt(selectedOption.dataset.duration);

    // Update the URL with the duration parameter
    updateUrlWithDuration(duration);
...
function updateUrlWithDuration(duration) {
    // Get the current URL
    var currentUrl = window.location.href;

    // Check if the URL already contains a query string
    if (currentUrl.includes('?')) {
        // URL already contains a query string, so update the duration parameter
        var urlParams = new URLSearchParams(window.location.search);
        ...
        // URL does not contain a query string, so add the query string with the duration parameter
        window.history.replaceState(null, null, currentUrl + '?duration=' + duration);
    }
}

// Show loading spinner when the page is reloading
document.addEventListener('DOMContentLoaded', function () {
...
// Hide loading spinner when the page has finished reloading
