...
if (isset($_GET['delete'])) {
   ...
   $update_admin_pass = $conn->prepare("UPDATE `bookings` SET bookingstat = ? WHERE booking_id = ?");
   $update_admin_pass->execute(["Batal", $delete_id]);
...
if ($_SESSION['paid'] == true) {
   ...
   $bookSuccessScript = "<script>
           window.onload = function() {
               Swal.fire({
                   title: 'Berjaya!',
                   text: 'Pembayaran telah berjaya dibuat! Sila ambil perhatian dengan tarikh slot rawatan. Terima Kasih.',
                   icon: 'success',
                   confirmButtonText: 'OK',
                   customClass: {
                       // Define a class for the SweetAlert popup
...
if ($_SESSION['paid'] == false) {
   // Prepare JavaScript for showing the Sweet Alert
   $bookSuccessScript = "<script>
           window.onload = function() {
               Swal.fire({
                   title: 'Tidak Berjaya!',
                   text: 'Pembayaran telah gagal dibuat! Sila cuba lagi atau hubungi kami untuk bantuan.',
                   icon: 'error',
                   confirmButtonText: 'OK',
                   customClass: {
                       // Define a class for the SweetAlert popup
   ...
   unset($_SESSION['fpaid']); // Unset the flag
} else {
}
...
<div id="bruv" class="section">
  ...
  <a  data-tippy-content="Batalkan Tempahan" href="booking-record.php?delete=' . $id . '" onclick="return confirm(\'Adakah anda pasti untuk batalkan tempahan ini? Maklumat berkaitan pengguna ini akan dibuang!\');" class="tblbtn tblremove"><i style="margin: 0em 0.2em;"class="fa fa-ban"></i></a>
