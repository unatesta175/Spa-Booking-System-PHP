...
function build_calendar($month, $year)
{
    ...
    // Prepare SQL statement to fetch staffs
    $stmt = $conn->prepare("SELECT * FROM staffs");
    $stmt->execute();

    // Fetch staffs and build staff select options
    $staffOptions = "";
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $staffOptions .= "<option value='" . $row['id'] . "'>" . $row['name'] . "</option>";
    }

    // Check if a staff ID is submitted
    $selectedStaffId = isset($_GET['staff_id']) ? $_GET['staff_id'] : null;

    // Prepare SQL statement to fetch bookings based on selected staff ID
    $stmt = $conn->prepare("SELECT * FROM bookings WHERE MONTH(date) = ? AND YEAR(date) = ? AND staff_id = ?");
    $stmt->execute([$month, $year, $selectedStaffId]);
    ...
    <form id='room_select_form' method='GET'>
        <div class='row'>
            <div class='col-md-6 col-md-offset-3 form-group'>
                ...
                <select class='form-control' id='staff_select' name='staff_id' onchange='this.form.submit()'>
