...
// Set cURL options
curl_setopt($curl, CURLOPT_URL, 'https://toyyibpay.com/index.php/api/createBill'); // Replace 'https://example.com/api' with your API endpoint URL
curl_setopt($curl, CURLOPT_RETURNTRANSFER, true); // Return response as string
curl_setopt($curl, CURLOPT_HEADER, true); // Include response headers
curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, 10); // Timeout in seconds
...
if ($response === false) {
    ...
    $responseData = json_decode($body, true); // Assuming the response is in JSON format

    // Process the API response here based on your application's requirements
}
