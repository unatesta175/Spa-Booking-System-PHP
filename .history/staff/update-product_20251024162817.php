...
if (isset($_POST['update'])) {
   ...
   if (!empty($image_01)) {
      if ($image_size_01 > 2000000) {
         ...
         $message[] = 'Gambar produk 01 berjaya dikemaskini!';
      }
   }
   ...
   $image_02 = htmlspecialchars(strip_tags(trim($image_02)), ENT_QUOTES, 'UTF-8');
   ...
   if (!empty($image_02)) {
      if ($image_size_02 > 2000000) {
         ...
         if (!empty($old_image_02) && file_exists('../uploaded_img/' . $old_image_02)) {
            unlink('../uploaded_img/' . $old_image_02);
         }

         $message[] = 'Gambar produk 02 berjaya dikemaskini!';
      }
   }

   $old_image_03 = $_POST['old_image_03'];
   $image_03 = $_FILES['image_03']['name'];
   $image_03 = htmlspecialchars(strip_tags(trim($image_03)), ENT_QUOTES, 'UTF-8');
   ...
   if (!empty($image_03)) {
      if ($image_size_03 > 2000000) {
         ...
         if (!empty($old_image_03) && file_exists('../uploaded_img/' . $old_image_03)) {
            unlink('../uploaded_img/' . $old_image_03);
         }

         $message[] = 'Gambar produk 03 berjaya dikemaskini!';
      }
   }
...
<body>
   ...
   <section class="update-product">
      ...
      <h1 class="heading">Kemaskini Produk</h1>
      ...
      if ($select_products->rowCount() > 0) {
         while ($fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)) {
            ...
            <form action="" method="post" enctype="multipart/form-data">
               ...
               <span>Kemaskini deskripsi produk : </span>
               <textarea name="details" class="box" required cols="30" rows="10"><?= $fetch_products['details']; ?></textarea>
               <span>Kemaskini gambar produk 01 : </span>
               <input type="file" name="image_01" accept="image/jpg, image/jpeg, image/png, image/webp" class="box">
               <span>Kemaskini gambar produk 02 : </span>
               <input type="file" name="image_02" accept="image/jpg, image/jpeg, image/png, image/webp" class="box">
               <span>Kemaskini gambar produk 03 : </span>
