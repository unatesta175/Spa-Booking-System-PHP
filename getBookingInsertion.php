...
if (!$booking_allowed) {
    ...
    $stmt = $conn->prepare("INSERT INTO `bookings` ( booking_id, timeslot, date, starttime, endtime, duration, datetimeapplied, updated_at, claimstat, bookingstat, pay_amount, pay_stat, client_id, service_id, staff_id) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $some_data = array(
                ...
                // 'billAmount' => $rmx100,
                'billAmount' =>  $bruh,
                'billReturnUrl' => (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/payment-success.php?booking_id=' . $last_inserted_id,
                'billCallbackUrl' => (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/payment-callback.php',
