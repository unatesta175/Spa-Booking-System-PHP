...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $date = $_POST['date'];
   $date = htmlspecialchars(strip_tags(trim($date)), ENT_QUOTES, 'UTF-8');
   $time = $_POST['time'];
   $time = htmlspecialchars(strip_tags(trim($time)), ENT_QUOTES, 'UTF-8');
   ...
   $pay_type = htmlspecialchars(strip_tags(trim($pay_type)), ENT_QUOTES, 'UTF-8');
      ...
      // Create event listener for both calendar
         ...
         const urlParams = new URLSearchParams(window.location.search);
               ...
               if (calendar1.view.type === 'timeGridDay' && slotLaneInfo.date.getDay() === 2) {
                  ...
                  return {
                                       html: '<div class="available-slot" style="color: #155724; background-color:#d4edda;border-color: #c3e6cb; text-align: center; height:100%; ">Available Slot</div>'
                  };
               } else if (
