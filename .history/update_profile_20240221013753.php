...
if (isset($_POST['submit'])) {
   ...
   $update = $conn->prepare("UPDATE `clients` SET name = ?, ic = ?, email = ?, password = ?, phoneno = ? ,faculty_id = ? , datebirth= ? , address= ? , maritalstat= ?, religion= ?, occupation= ?, company= ?, fb= ?, ig= ?, allergy= ? WHERE id = ?");
   $update->execute([$name,, $ic$email, $studno, $program_id, $faculty_id, $part, $phoneno, $homephoneno, $user_id]);
