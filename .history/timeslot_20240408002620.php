...
$stmt = $conn->prepare("SELECT * FROM bookings WHERE date = ?"); // Assuming you have a 'date' field in your bookings table
$stmt->execute([$date]);
...
function timeslots($duration, $cleanup, $start, $end, $existingBookings)
{
    ...
    for ($currentTime = $startTime; $currentTime < $endTime; $currentTime += ($duration + $cleanup) * 60) {
        ...
        if (!$overlap) {
            // Format timeslot start and end times
            $slotStartTime = date('h:ia', $currentTime);
            $slotEndTime = date('h:ia', $endTimeSlot);
            ...
            foreach ($timeslots as $ts) {
                ...
                <div class="col-md-2">
                    <div class="form-group m-3">
                        ...
                        // Display the button based on booking status
                        if ($isBooked) { ?>
                            <button class="btns btn-danger book" disabled><?php echo $ts; ?></button>
                        <?php } else { ?>
