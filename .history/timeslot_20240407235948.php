...
function timeslots($duration, $cleanup, $start, $end, $existingBookings)
{
    ...
    for ($intStart = $start; $intStart < $end; $intStart->add($interval)->add($cleanupInterval)) {
        ...
        foreach ($existingBookings as $booking) {
            $bookingStartTime = new DateTime($booking['time']);
            $bookingEndTime = (new DateTime($booking['time']))->add(new DateInterval("PT" . $booking['duration'] . "M"));
            if (($intStart >= $bookingStartTime && $intStart < $bookingEndTime) || 
                ($endPeriod > $bookingStartTime && $endPeriod <= $bookingEndTime)) {
                $overlap = true;
                break;
            }
        }
