...
foreach ($timeslots as $ts) {
    // Extract the start time and AM/PM information from the timeslot string
    $start_parts = explode(' - ', $ts['timeslot'])[0];
    $start_time = substr($start_parts, 0, 5); // Extracting only the time part
    $am_pm = substr($start_parts, -2); // Extracting AM/PM part
    ?>
    <div class="col-md-2">
        <div class="form-group m-3">
            ...
            if ($ts['overlap']) { ?>
                <button class="btns btn-danger book" data-timeslot="<?php echo $ts['timeslot']; ?>">
                    <?php echo $start_time . ' ' . $am_pm; ?> <!-- Display start time with AM/PM -->
                </button>
            <?php } else { ?>
