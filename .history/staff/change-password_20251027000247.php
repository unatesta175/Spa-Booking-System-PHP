...
if (isset($_POST['submit'])) {
   ...
   $new_pass = htmlspecialchars(strip_tags(trim($new_pass)), ENT_QUOTES, 'UTF-8');
   $cpass = ($_POST['cpass']);
   $cpass = htmlspecialchars(strip_tags(trim($cpass)), ENT_QUOTES, 'UTF-8');

   if ($old_pass == $empty_pass) {
      $error_message = 'Sila masukkan kata laluan lama!';
   } elseif ($old_pass != $prev_pass) {
      $error_message = 'Kata laluan lama tidak sepadan!';
   } elseif ($new_pass != $cpass) {
      $error_message = 'Kata laluan pengesahan tidak sepadan!';
   } else {
      if ($new_pass != $empty_pass) {
         $update_admin_pass = $conn->prepare("UPDATE `staffs` SET password = ? WHERE id = ?");
         $update_admin_pass->execute([$cpass, $staff_id]);
         $success_message = 'Kata laluan berjaya dikemaskini!';
      } else {
         $error_message = 'Sila masukkan kata laluan baru!';
      }
   }
}
...
<body>
   ...
   <script>
      const passwordInputx = document.getElementById("old_pass");
   ...
   <script>
      const passwordInputy = document.getElementById("new_pass");
   ...
   <script>
      const passwordInputz = document.getElementById("cpass");
