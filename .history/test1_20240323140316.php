...
<body>
  <section>
    ...
    <form id="step2" style="display: none;" method="post" action="">
        ...
        <div class="form-group">
            <div class="inputBox">
                ...
                <input type="text" name="remarks" required placeholder="Masukkan pesanan anda" maxlength="200" class="box">
            </div>
    ...
    <form id="step3" style="display: none;">
        <h2>Step 3</h2>
        <div class="form-group">
            <!-- Hidden inputs to store values from form1 and form2 -->
...
<script>
    $(document).ready(function() {
        // Function to update hidden inputs in form3 with values from form1 and form2
        function updateHiddenInputs() {
            $('#date').val($('input[name="date"]').val());
            $('#time').val($('input[name="time"]').val());
            $('#remarks').val($('input[name="remarks"]').val());
            $('#staff_id').val($('select[name="staff_id"]').val());
            // Update other hidden inputs as needed
        }

        // Next button click handler for moving to the next step
        $('.next').click(function() {
            if ($(this).closest('form')[0].checkValidity()) {
                updateHiddenInputs(); // Update hidden inputs
                $(this).closest('form').hide().next('form').show(); // Show next form
            } else {
                // Handle validation errors if any
            }
        });

        // Previous button click handler for moving to the previous step
        $('.prev').click(function() {
            $(this).closest('form').hide().prev('form').show(); // Show previous form
        });
    });
</script>
  </script>
